...
- (void)viewDidLoad {
     ...
     */
    [ZWCommonMenuView createMenuWithFrame:CGRectZero target:self dataArray:dataArray itemsClickBlock:^(NSString *str, NSInteger tag) {
...
- (void)popMenu:(CGPoint)point{
    if (self.flag) {
        [ZWCommonMenuView showMenuAtPoint:point];
        self.flag = NO;
    }else{
...
- (IBAction)addMenuItem:(id)sender {
     ...
     *  追加菜单项
     */
    [ZWCommonMenuView appendMenuItemsWith:newItemArray];
...
- (IBAction)removeMenuItem:(id)sender {
     ...
     *  更新菜单
     */
    [ZWCommonMenuView updateMenuItemsWith:_dataArray];
...
- (void)dealloc{
    [ZWCommonMenuView clearMenu];   // 移除菜单
}
